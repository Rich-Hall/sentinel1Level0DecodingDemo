...
## A note on data
Sentinel-1 Level 0 files are typically too large to reside on github - the file used in the example is 660MB, against github's upload limit of 100MB. It is therefore assumed that the user has downloaded their own data, likely from ESA's [Copernicus Browser](https://browser.dataspace.copernicus.eu).

The particular stripmap data take used in the example was acquired on 26-Dec-2025 over Sao Paulo by Sentinel-1A. The search term "S1A_S6_RAW__0SDV_20251226T214356_20251226T214427_062491_07D496_02D3" (without the quotes) on the Copernicus site should find this product. A user account will be required to download the data, and the user will need to unzip the archive once downloaded. The very long folder names retained in the archive's file structure tend to produce problems, so it is assumed the .dat files inside the archive are then placed in the data/sao_paulo folder directly (without being renamed).
